...
class Pygame_utils:
        ...
        for row in self.laberinto:
            for col in row:
                if col == "#":
                    ...
                    if (self.counter % 2) == 0:
                        self.bot_rect_list.append([pygame.transform.scale(pygame.image.load("code_images/benderdos.png"), (width, width)),(x,y)])
                    else:
                        self.bot_rect_list.append([pygame.transform.scale(pygame.image.load("code_images/benderuno.png"), (width, width)), (x,y)])
                    self.counter+=1
                x += width                
            y += width
            x = (self.x/2)-(x_len/2)*width
    ...
    def completado(self):
        ...
        bender=pygame.transform.scale(pygame.image.load("code/images/bender_final.png"), (width, width))
